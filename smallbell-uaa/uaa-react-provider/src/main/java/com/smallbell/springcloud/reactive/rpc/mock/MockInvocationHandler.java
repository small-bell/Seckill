package com.smallbell.springcloud.reactive.rpc.mock;
...
class MockInvocationHandler implements InvocationHandler
{
    ...
    public static <T> T newInstance(Class<T> clazz)
    {
        ...
        /**
         *  通过反射，迭代远程调用接口中的每一个方法，组装对应的 ReactiveRpcMethodHandler 模拟方法处理器
         */
        for (Method method : clazz.getMethods())
        {
            ...
            // 得到返回对象的实际类型
            //getGenericReturnType 获取到泛型信息
            Class<?> returnValueType = extractElementType(method.getGenericReturnType());

            /**
             * 组装 ReactiveRpcMethodHandler 模拟方法处理器
             * 注入 REST 地址的 contextPath 部分和uri部分
             */
            ReactiveRpcMethodHandler handler = new ReactiveRpcMethodHandler(contextPath, uri, returnType,returnValueType);
     ...
     * @return
     */
  static   private Class<?> extractElementType(Type genericReturnType)
    {
